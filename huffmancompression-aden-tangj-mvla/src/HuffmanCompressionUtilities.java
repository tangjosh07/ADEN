...
public class HuffmanCompressionUtilities {
	...
	int[] readFreqWeights(File inf) {
		...
		try {
			while ((line = br.readLine()) != null) {
				splits = line.split(",");
				weight[Integer.parseInt(splits[0])] = Integer.parseInt(splits[1]);
			}
			br.close();
	...
	void initializeHuffmanQueue(boolean minimize) {
		for (int i = 0; i < weights.length; i++) {
			if (weights[i] == 0 && minimize) continue;
	...
	void dumpQueue(String msg) {
		...
		Queue<HuffmanTreeNode> saveQ = new LinkedList<>();
	...
	private int traverseTree(HuffmanTreeNode root, String binStr, int index) {
		if (root.isLeaf()) return root.getOrdValue();
		if (index == binStr.length()) return -1;
		char c = binStr.charAt(index);
		if (c == '0') return traverseTree(root.getLeft(),binStr, index+1);
		else return traverseTree(root.getRight(),binStr,index+1);
	}
